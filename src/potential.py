    ...
    def loop(self):
        ...
        if getDistanceInMeters(boat, goal) > 5: 
            ...
            for angle in range(0, 360, 2):
              ...
              GoalP = Potentials.PotentialGoal(pot,position, goal)
              ObstacleP = Potentials.PotentialObstacle(pot,position, obstacle)
              UpwindP = Potentials.PotentialUpwind(pot,boat, position, windDbw)
              DownwindP = Potentials.PotentialDownwind(pot,boat, position, windDbw)
              AngleChangeP = Potentials.PotentialAngleChange(pot,boat, position, windDbw, course)
        ...
        pubcourse(x)
        print('Obliczona wartosc zadana kursu wynosi:', x)
