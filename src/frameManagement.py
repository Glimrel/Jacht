...
def parseBasicSensor(frame):
    ...
    LongX = int(LongitudeFloat[0]/100) * 100
    LongXD = (LongitudeFloat[0] - LongX) * 100 / 60
    LongDXD = LongX + LongXD
    msg.longitude = LongDXD
    
    LatiX = int(LatitudeFloat[0]/100) * 100
    LatiXD = (LatitudeFloat[0] - LatiX) * 100 / 60
    LatiDXD = LatiX + LatiXD
    msg.latitude = LatiDXD
...
def pubReset(msg):
    publisher = rospy.Publisher('reset2', Bool, queue_size=10)
    publisher.publish(msg)
